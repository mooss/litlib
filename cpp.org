...
using namespace std;
// Cannot make the container one with a function, but it works perfectly with a lambda.
...
// Vulgar attempt to identify a container.
template<class T>
requires std::same_as<decltype(std::declval<T>().cbegin()), typename T::const_iterator>
void one(const T& container) {
...
#+end_src
#+depends:panic :noweb foreach :cpp iostream

#+begin_src cpp :eval no-export :exports both :results silent
<<include(":noweb panic foreach :cpp vector")>>
int main() {
...
#+end_src
#+depends:vecmaybe :noweb maybe :cpp vector string
