...
This code block has a special name =_include= - prefixed by an underscore - to avoid clashing with other documents that will implement their own =include= code block.
...
<<_include(noweb="type_str", cpp="iostream")>>
using namespace std;
int main(){
    cout << type_str<int>() << "\n";
    const long ft = 42;
