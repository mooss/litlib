...
struct maybe {
    std::variant<Value, sterr> var;
    maybe(Value val): var(val) {}
    maybe(sterr val): var(val) {}
    explicit operator bool() const { return std::holds_alternative<Value>(var); }

    void assert() {
        if(!*this) {
            panic(
                std::get<sterr>(var).msg,
                "Hint: check if a maybe contains an sterr with the boolean conversion operator."
            );
        }
    }
    ...
    std::string_view err() const {
        ...
        return std::get<sterr>(var).msg;
    }
};
#+end_src
#+depends:maybe :noweb panic :cpp variant string string_view
