...
An inventory has the following constraints:
 ...
 3. Elements can be removed from anywhere without needing to reallocate them.
...
Usage:
#+begin_src cpp :eval no-export :exports both
<<include(":noweb inventory print-function :cpp functional")>>
...
The assignment operators is where all the magic occurs since they allow the stored value to be updated as if it was of type =Observed= and then notifies the observers.
Thus for example, an =obvervable<int>= can be updated as if it was actually =int= and trigger a notification to all observers.

#+begin_src cpp :noweb-ref observable/public :minipage
observable& operator=(Observed&& new_value) {
    observed_ = std::forward<Observed>(new_value);
    notify();
    return *this;
}
...
#+begin_src cpp :eval no-export :exports both
<<include(":noweb observable print-function print_info <<vector :cpp vector")>>
...
int main() {
    ...
    print{"\n||| Types information |||"};
    print_info<decltype(infix_sub)>();
    print_info<decltype(number)>();
    print_info<decltype(number)::observer_storage>();
}
#+end_src
...
||| Types information |||
subscription<observable<Observed>::observe<void (&)(int)>::<lambda(s)> > -> 24 bytes
observable<int> -> 32 bytes
std::function<void(const int&)> -> 32 bytes
#+end_src
