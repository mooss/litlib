...
The invariant of a segmented vector is maintained by =push_back='s requirement that only allows it to accept =SegmentSize= argument, translating into =SegmentSize= calls to the vector's own =push_back=.
It should be noted that nothing prevents the caller to call =push_back= manually on =storage=, since it is publicly accessible.
