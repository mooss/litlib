...
This struct stores references of callable that can then be sequentially called as if there was only one callable.

Some notes:
 - Ideally the functions would be stored by value when receiving a rvalue reference but I don't know how to do that right now.
   This means that the functions must be stored elsewhere.
...
struct funseq {
    ...
    funseq(Functions&... _functions):
        functions(_functions...)
    {}

    template<class... Args>
    void operator()(Args&... args) {
        foreach_tuple([&](auto& fun) {fun(args...);}, functions);
    }
};
#+end_src
#+depends:funseq :noweb foreach_tuple :cpp tuple

Usage:
#+begin_src cpp :eval no-export :exports both
<<include(":noweb print funseq")>>
...
int main() {
    auto print_pipes = [](const auto& el) {print(el, "||");};
    print_chevron chev{};
    funseq piloch(chev, print_pipes, print_lost);
    piloch("constant");
}
#+end_src
