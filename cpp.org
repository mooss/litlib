...
The =anchor= class is a pretty weird usage of C++.
The goal is to have a function returning an object that will perform an action on destruction but that action depends on whether the object was ignored.
...
A problem with this idea is that in order to known when it has been assigned, its implementation relies on side effect enacted by copy constructors.
...
Before implementing I want to illustrate how RVO is bypassed with =volatile=.
To this end, below is a struct that updates its state everytime a relevant event occurs and also prints said event.
...
talkative non_volatile_return(std::string const& initial) {
    talkative result(initial);
    return result;
}

talkative volatile_return(std::string const& initial) {
    volatile talkative result(initial);
    return result;
}

int main() {
    print{"1. ||| Non volatile captured |||"};
    auto x_nvr = non_volatile_return("Non volatile captured");

    print{"\n2. ||| Volatile captured |||"};
    auto y_vr = volatile_return("Volatile captured");

    print{"\n3. ||| Volatile ignored |||"};
    volatile_return("Volatile ignored");

    print{"\n4. ||| Types |||"};
    print{"Type of x_nvr:", type_str<decltype(x_nvr)>()};
    print{"Type of non volatile in y_vr:", type_str<decltype(y_vr)>()};
    volatile talkative explicitely_volatile("ignoreme");
    print{"Type of explicitely volatile talkative:", type_str<decltype(explicitely_volatile)>()};
...
#+RESULTS:
#+begin_src text
1. ||| Non volatile captured |||
Non volatile captured
...
4. ||| Types |||
Type of x_nvr: talkative
Type of non volatile in y_vr: talkative
ignoreme
Type of explicitely volatile talkative: volatile talkative

5. ||| End of scope |||
destroying: ignoreme
destroying: Volatile captured, volatile-move_c
destroying: Non volatile captured
#+end_src

To summarise the output above:
 1. The captured non volatile value inside =x= generates no side effect because of NRVO.
 2. The captured volatile value is moved into =y= and the original object is destroyed.
 3. The ignored volatile value is moved, uh, somewhere and both are destroyed.
 4. =x= and =y= are not volatile.
    This might explain what happens with the ignored volatile value.
    Since the return type of =volatile_return= is not qualified with =volatile= and that the compiler is not supposed to optimise away volatile values, the volatile move constructor is called in order to match the specified return type.
 5. The destructors are called at the end of the scope.


Interestingly, even though =talkative='s copy constructor is not used, it is still required in order to compile, probably because of the rules dictating when constructors are to be or not to be generated.
