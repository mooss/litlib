...
class inventory {
  public:
    ...
    using handle = typename warehouse::iterator;
...
=sub= represents a subsequence within a sequence, as a position an a length.
The constant =npos= is an invalid position, and since it is the default value, the default =sub= object can be used to convey a failure.

#+name: sub
#+begin_src cpp
struct sub {
    static constexpr std::size_t npos = 18446744073709551615UL; // Not ideal.
    std::size_t pos = npos;
    std::size_t len = 0;
};
#+end_src
...
#+name: foreach
#+begin_src cpp
void foreach(auto fun, const auto& range) {
    for(auto el: range) fun(el);
}
#+end_src
...
Keep inside a container the values respecting a predicate, modifying the container in place.
This implementation is quite naive and probably only works for vectors.

#+name: keepin
#+begin_src cpp
template<class Callable, class Container>
Container& keepin(Callable predicate, Container& sequence) {
    auto from = sequence.begin();
    auto to = from;
    for(; from != sequence.end(); ++from) {
        if(predicate(*from)) {
            if(from != to) {
                *to = *from;
            }
    ...
    if(from != to) {
        sequence.erase(to, sequence.end());
    }
    return sequence;
}
#+end_src
...
Usage:
#+begin_src cpp :eval no-export :exports both
<<include(":noweb keepin <<vector print :cpp vector")>>

int main() {
    std::vector<int> lost{4, 8, 15, 16, 23, 42};
    print{lost};
    auto even = [](int el) { return el%2 == 0; };
    auto evenlost = lost;
    print{keepin(even, evenlost)};

    auto oddlost = lost;
    auto odd = [](int el) { return el%2 != 0; };
    print{keepin(odd, oddlost)};
}
#+end_src
...
=slurp= reads a file into a =std::string=:
#+name: slurp
#+begin_src cpp
std::string slurp(const char* filename) {
    ...
    std::rewind(file);
    std::fread(res.data(), sizeof(char), size, file);
    std::fclose(file);
    return res;
}
#+end_src
#+depends:slurp :cpp cstdio string

Usage:
#+begin_src cpp :eval no-export :exports both
<<include(":noweb print slurp")>>

int main() {
    auto cpp_org = slurp("cpp.org");
    print(cpp_org.substr(2, 17));
}
#+end_src
...
=panic= prints its arguments to stderr and exits.
#+name: panic_impl
#+begin_src cpp
namespace panic_impl {
// This lambda can also be used in extensions, like panic/vector.
auto one_ = [](const auto& el) {
    std::cerr << el << "\n";
};
void one(const auto& el) { one_(el); }
} // namespace panic_impl
#+end_src
#+depends:panic_impl :noweb foreach :cpp iostream
#+name: panic
#+begin_src cpp
namespace panic_impl {
void all(){}
void all(const auto& head, const auto&... tail) {
    one(head);
    all(tail...);
}
}
...
#+end_src
#+depends:panic :noweb panic_impl

Usage:
#+begin_src cpp :eval no-export :exports both :results silent
<<include(":noweb panic :cpp vector")>>
int main() {
...
#+name: panic_impl/vector
#+begin_src cpp
namespace panic_impl {
template<class T, class Allocator>
void one(const std::vector<T, Allocator>& container) {
    foreach(one_, container);
}
} // namespace panic_impl
#+end_src
#+depends:panic_impl/vector :noweb panic_impl foreach :cpp vector
#+depends:panic/vector :noweb panic_impl/vector panic

#+begin_src cpp :eval no-export :exports both :results silent
<<include(":noweb panic/vector foreach :cpp vector")>>
int main() {
    std::vector<std::string> lost{"4", "8", "15", "16"};
    panic("something", "terrible", "happened!", lost);
}
#+end_src

*** =maybe=

Maybe can contain a value or an error.
#+name: maybe
#+begin_src cpp
struct sterr {
    std::string msg;
...
template<class Value, class Error=sterr>
class maybe {
  private:
    std::variant<Value, Error> var;
    void private_nofail() const {
        if(!*this) panic(
            std::get<Error>(var).msg,
            "Hint: check if a maybe contains an error with the boolean conversion operator."
        );
    }
  ...
  public:
    ...
    template<class V>
    maybe(V&& val): var(std::forward<V>(val)) {}
    explicit operator bool() const { return std::holds_alternative<Value>(var); }

    Value& operator*() {
        private_nofail();
        return std::get<Value>(var);
    }
    ...
    const auto& msg() const {
        auto error = err();
        if(error == nullptr) {
            panic("Tried to access a non-existing error message.");
        }
        return error->msg;
    }

    void nofail() const {
        if(!*this) panic(std::get<Error>(var).msg);
    }
};
#+end_src
#+depends:maybe :noweb panic :cpp variant string

Some utilities that will be used at various points to illustrate =maybe= and its derivatives:
#+name: maybe-functions
#+begin_src cpp
maybe<int> even(int i) {
    if(i%2 == 0) {
        return i;
    }
    return sterr{"expected an even number"};
}

void print_maybe(const auto& arg) {
    if(arg) {
        print{*arg};
    } else {
        print{arg.msg()};
...
#+end_src
#+depends:maybe-functions :noweb print maybe

Usage:
#+begin_src cpp :eval no-export :exports both
<<include(":noweb maybe-functions print_size")>>

int main() {
    print{*even(16)};

    // print{*even(23)};
    // Exits and prints to stderr:
    // expected an even number
    // Hint: check if a maybe contains an error with the boolean conversion operator.

    auto fifteen = even(15);
    if(fifteen) {
        print{*fifteen};
    } else {
        print{fifteen.msg()};
...
16
expected an even number
maybe<int> -> 40 bytes (320 bits)
std::__cxx11::basic_string<char> -> 32 bytes (256 bits)
#+end_src

*** =vecmaybe=

=vecmaybe= can be used to represent multiple errors.

#+name: vecmaybe
#+begin_src cpp
struct vecerr {
    std::vector<std::string> msg;
    explicit operator bool() const { return !msg.empty(); } // Makes it easy to test if errors are stored.
    void append(const auto& el) { msg.push_back(el.msg()); } // Append errors from maybe<T, sterr>.
};

template<class T>
using vecmaybe = maybe<T, vecerr>;
#+end_src
#+depends:vecmaybe :noweb panic/vector maybe :cpp vector string

Usage:
#+begin_src cpp :eval no-export :exports both
<<include(":noweb <<vector maybe-functions vecmaybe")>>

vecmaybe<std::vector<int>> veceven(const std::vector<int>& vec) {
    vecerr error;
    std::vector<int> success;
    for(auto el: vec) {
        auto maybe_even = even(el);
        if(maybe_even) {
            if(!error) success.push_back(*maybe_even);
        } else {
            error.append(maybe_even);
        }
    }
...
int main() {
    print_maybe(veceven({4, 8, 15, 16, 23, 42}));
    print_maybe(veceven({4, 8, 16, 42, 108}));
}
#+end_src

#+RESULTS:
#+begin_src text :minipage
[expected an even number, expected an even number]
[4, 8, 16, 42, 108]
#+end_src

*** =maperr=

This strange beast attempts to range over a container and to build a vector of successes, returning the first error that comes up.
There are lots of way to improve it, starting with moving values out of =maybe_res=.

#+name: maperr
#+begin_src cpp
template<class C>
auto maperr(auto fun, const C& container) {
    using T = decltype(fun(std::declval<typename C::value_type>()))::value_type;
    using vec = std::vector<T>;
    vec success;
    success.reserve(container.size());
    for(auto& el: container) {
        auto maybe_res = fun(el);
        if(!maybe_res) {
            return maybe<vec>(*maybe_res.err());
        }
        success.push_back(*maybe_res);
    }
    return maybe<vec>(std::move(success));
}
#+end_src
#+depends:maperr :noweb maybe :cpp vector

Usage:
#+begin_src cpp :eval no-export :exports both
<<include(":noweb <<vector maybe-functions maperr print_size")>>

int main() {
    // TODO find how to implicitly "cast" initialiser lists to vectors in maperr.
    auto res = maperr(even, std::vector<int>{4, 8, 15, 16, 23, 42});
    print_size<decltype(res)>();
    print_maybe(res);
    print_maybe(maperr(even, std::vector<int>{4, 8, 16, 42, 108}));
}
#+end_src

#+RESULTS:
#+begin_src text :minipage
maybe<std::vector<int>, sterr> -> 40 bytes (320 bits)
expected an even number
[4, 8, 16, 42, 108]
#+end_src
...
A simple wrapper around =maperr= that triggers a panic via =maybe='s =nofail= should an error be encountered.

#+name: mapassert
#+begin_src cpp
auto mapassert(auto fun, const auto& container) {
    auto res = maperr(fun, container);
    res.nofail();
    return *res; // TODO: move into local and return for RVO ?
}
#+end_src
#+depends:mapassert :noweb maperr

Usage:
#+begin_src cpp :eval no-export :exports both
<<include(":noweb <<vector maybe-functions mapassert print_size")>>

int main() {
    // Panics with a single message (expected an even number):
    // print{mapassert(even, std::vector<int>{4, 8, 15, 16, 23, 42})};
    print{mapassert(even, std::vector<int>{4, 8, 16, 42, 108})};
}
#+end_src
...
A finder is a function able to find a subsequence with a string view.
Such subsequence will be called a match.

The function below builds a finder returning a subsequence starting at the first character respecting =predicate=.
This subsequence is extended to all following characters respecting =predicate=.
#+name: greedy_finder
#+begin_src cpp
auto greedy_finder(auto predicate) {
    return [=](const std::string_view s) {
        for(std::size_t begin = 0; begin < s.size(); ++begin) {
            if(predicate(s[begin])) {
                auto end = begin;
                while(++end < s.size() && predicate(s[end]));
...
#+end_src
#+depends:greedy_finder :noweb sub :cpp string_view

*** Split

This function splits a =std::string_view=, using a finder able to find the next delimiter.
This approach should allow to specify a lot of useful splitting behaviors.

#+begin_src cpp :noweb-ref split/sv
template<class Finder>
requires std::same_as<sub, decltype(std::declval<Finder>()(std::declval<std::string_view>()))>
std::vector<std::string_view> split(std::string_view source, Finder finder) {
    std::vector<std::string_view> res;
    while(true) {
        sub delim = finder(source);
        if(delim.pos == sub::npos) { // Nothing found.
            res.push_back(source);
            break;
        }
        res.push_back(source.substr(0, delim.pos));
        source = source.substr(delim.pos + delim.len);
        if(source.size() == 0) { // Source ends with a delimiter.
            res.push_back(source);
            break;
        }
    }
...
#+end_src
#+depends:split/sv :noweb sub :cpp string_view vector concepts

This split on the given delimiter and is a good example on how to use the functional =split= function:
#+begin_src cpp :noweb-ref split/sv
std::vector<std::string_view> split(const std::string_view source, const std::string_view delim) {
    return split(source, [delim](const std::string_view _source){
        auto pos = _source.find(delim);
        if(pos == _source.npos) {
            return sub{};
        }
        return sub{.pos=pos, .len=delim.size()};
    });
}
#+end_src
...
Usage:
#+begin_src cpp :eval no-export :exports both
<<include(":noweb <<vector print split/sv greedy_finder")>>

bool is_slash(char c) { return c == '/'; }

int main() {
    print{split(" -> 4 -> 8 -> 15 -> 16 -> 23 -> 42 -> ", " -> ")};
    print{split("4 -> 8 -> 15 -> 16 -> 23 -> 42", " -> ")};
    print{split("//4/8/////15///16////////23/42/", greedy_finder(is_slash))};
}
#+end_src
...
The conversions functions defined in this section are an attempt to bypass the limitation of the =ato*= function (their usage of 0 to signify an error), without using function that throw exceptions.

A problem that still needs to be solved is the fact that a =std::string_view= can hold non null-terminated data, and the =ato*= functions cannot handle that.
Another problem is that there is no way to know where the number parsing stopped.
...
maybe<float> to_float(const std::string_view str) {
    ...
    if(res == 0 && str != "0" && str != "0.0" && str != ".0") {
        return sterr{"failed to parse value as float"};
    }
    return float(res);
}
...
maybe<unsigned int> to_uint(const std::string_view str) {
    ...
    if(res < 0) {
        return sterr{"expected an unsigned int, got negative number"};
    }
    if(res == 0 && str != "0") {
        return sterr{"failed to parse value as unsigned int"};
    }
    return (unsigned int) res;
}
#+end_src
#+depends:to-numbers :noweb maybe :cpp string_view cstdlib

#+begin_src cpp :eval no-export :exports both
<<include(":noweb print to-numbers")>>

int main() {
    print{*to_uint("4")};
    print{to_uint("-8").msg()};
    print{*to_float("15b")}; // No way to know parsing happily stopped at b.
    print{to_float("#16").msg()};
}
#+end_src
...
#+begin_src text :minipage
4
expected an unsigned int, got negative number
15
...
The =dynasty= type is a way to create a chain of inheritance using CRTP.
The syntax for using it is =dynasty<founder, member1, member2, ..., memberN>=.

The use case is to define a founder type, which holds all the data as well as common operations.
Subsequents data-free members can be added to the =dynasty=, supplying it with additional operations.
In the end, this provides an easy way to define a serie of advanced behaviours to a base type.

The non-founding members of a dynasty must provide a templated =heir= that will inherit from its single template parameter.
Here is an example of such a member:
#+begin_src cpp
struct example {
    template<class Ancestor>
    struct heir: Ancestor {
        void out() {print{this->repr()};}
    };
};
#+end_src

In the implementation, the list of members of this dynasty is reversed so that when using =dynasty=, the inheritance is defined left to right rather than right to left.
The right to left definition would otherwise be forced on us because of the fact that variadic template parameters must apparently appear last (I could not make =struct dynasty<Ancestors..., Ruler>= work).

#+name: dynasty
#+begin_src cpp
namespace dynasty_impl {
template<class... Ancestors> struct reversed_dynasty;

template<class Founder>
struct reversed_dynasty<Founder>: public Founder {
    using founder = Founder;
    reversed_dynasty(Founder&& f):
        Founder(std::move(f))
    {}
};

template<class Ruler, class... Ancestors>
struct reversed_dynasty<Ruler, Ancestors...>: Ruler::heir<reversed_dynasty<Ancestors...>> {
    using super = typename Ruler::heir<reversed_dynasty<Ancestors...>>;
    using founder = super::founder;
    reversed_dynasty(super::founder&& f):
        super(std::move(f))
    {}
};

template<class... Members>
using unreversed_dynasty = meta::steal<reversed_dynasty, meta::reverse<Members...>>;

template<class Founder, class... Members>
struct dynasty: unreversed_dynasty<Founder, Members...> {
    using super = unreversed_dynasty<Founder, Members...>;
    using founder = super::founder;
    dynasty(founder&& f):
        super(std::move(f))
    {}
...
template<class... Members>
using dynasty = dynasty_impl::dynasty<Members...>;// meta::steal<dynasty_impl::dynasty, meta::reverse<Members...>>;
#+end_src
#+depends:dynasty :noweb meta::steal meta::reverse

*** Lineage

=dynasty= can be inconvenient to use because the members must be wrapped in an external type, which is not always needed.
When that is the case, =lineage= can take care of wrapping its members in a struct that will provide the =heir=.

#+name: lineage
#+begin_src cpp
namespace lineage_impl {
template<template <class> class Member>
struct wrap_heir {
    template<class Ancestor>
    using heir = Member<Ancestor>;
};

template<template<class> class... Members>
struct heir_wrapper;

template<template <class> class Member>
struct heir_wrapper<Member> {
    using type = meta::list<wrap_heir<Member>>;
};

template<template <class> class Head, template <class> class... Tail>
struct heir_wrapper<Head, Tail...> {
    using type = meta::cat<wrap_heir<Head>, typename heir_wrapper<Tail...>::type>;
};
}

template<class Founder, template <class> class... UnwrappedAncestors>
using lineage = meta::steal<
    dynasty_impl::dynasty,
    meta::cat<Founder, typename lineage_impl::heir_wrapper<UnwrappedAncestors...>::type>
>;
#+end_src
#+depends:tmpl_dynasty :noweb meta::list meta::cat dynasty meta::reverse

*** Usage

#+begin_src cpp :eval no-export :exports both
<<include(":noweb print print_size dynasty lineage :cpp string")>>

struct presuf {
    const std::string pref;
    const std::string suff;
    presuf(const std::string& a, const std::string& b):
        pref(a), suff(b)
    {}
    std::string op(const std::string& arg) const {return pref + arg + suff;}
};
...
struct stb: Ancestor {
    void perform() const {
        print{this->op(" perform")};
    }
};

int main() {
    dynasty<presuf, st, add_newline> lost({"lost", " xxx"});
    lost.act();
    lineage<presuf, stt, stb> last({"last", " zzz"});
    last.act();
    last.perform();
    print{};
    print_size(lost); print_size(last);
    print_size<std::string>();
}
#+end_src
...
#+RESULTS:
#+begin_src text :minipage
lost 4 8 15 16 23 42 xxx

last act zzz
last perform zzz

dynasty_impl::dynasty<presuf, st, add_newline> -> 64 bytes (512 bits)
dynasty_impl::dynasty<presuf, lineage_impl::wrap_heir<stt>, lineage_impl::wrap_heir<stb> > -> 64 bytes (512 bits)
std::__cxx11::basic_string<char> -> 32 bytes (256 bits)
#+end_src
