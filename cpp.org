...
#+begin_src cpp :minipage
namespace meta {
template<template<typename...> class Garment, typename Typelist>
struct dress_impl;
...
This is similar to a =map= function, except that instead of applying a function to every element of a data structure, a template template is applied to every element in the metalist.
...
#+begin_src cpp :minipage
namespace meta {
template<template<typename...> class Stealer, typename Robbed>
struct steal_impl;
...
namespace meta {
template<
    template<typename...> class Guild,
    template<typename...> class Stealer,
    typename... UnsuspectingTargets
>
struct stealall_impl {
    template<typename Robbed>
...
int main() {
    print_type<meta::stealall<std::tuple, std::list, std::vector<int>, std::vector<float>, std::vector<bool>>>();
}
#+end_src
...
Usage:
#+begin_src cpp :eval no-export :flags -Wall -std=c++17 :noweb no-export :exports both :minipage
