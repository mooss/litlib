...
I didn't like the idea of using =std::accumulate= with a begin iterator, a end iterator and a mandatory neutral element so I made the following =sum= template for a more Python-like experience.
...
This template is not compatible with C arrays.
It would perhaps be better, cleaner and compatible with C arrays to use =std::span= rather than a templated function.
I don't know the implications but I might consider it later.

Usage example:
#+begin_src cpp :eval no-export :flags -Wall -std=c++17 :noweb no-export
<<include(":noweb sum print-function :cpp vector array")>>

int main() {
    std::vector<int> lost_v{4, 8, 15, 16, 23, 42};
    std::array<int, 6> lost_a{4, 8, 15, 16, 23, 42};
    print{"Sum of lost numbers with a std::vector:", lit::sum(lost_v)};
    print{"Same thing with a std::array:", lit::sum(lost_a)};
}
#+end_src
