...
The iterators only have the bare minimum required to be usable in a range-based for loop.
...
struct Q_iterator {
    ...
    bool operator!=(const Q_iterator& other) {
...
struct linear_interpolation {
    ...
    using data_point = std::span<value_type, segment_size>;
