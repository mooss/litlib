...
To avoid having to manually construct a =std::span= to print a vector or an array, shortcuts are baked into the blocks =<<vector= and =<<array=.

#+name: <<vector
#+begin_src cpp
template<class T, class Allocator>
std::ostream& operator<<(std::ostream& os, const std::vector<T, Allocator>& container) {
    return os << std::span(container);
}
#+end_src
#+depends:<<vector :noweb <<span :cpp vector span
...
#+end_src
#+depends:<<array :noweb <<span :cpp array span

Usage:
#+begin_src cpp :eval no-export :exports both
<<include(":noweb <<vector <<array :cpp iostream")>>

int main() {
    std::vector<int> lost_vector{4, 8, 15, 16, 23, 42};
    std::array<int, 6> lost_array{4, 8, 15, 16, 23, 42};
    std::cout << "vector: " << lost_vector << "\n";
    std::cout << "array: " << lost_array << "\n";
}
#+end_src
...
=finaliser= is a class that can be returned from a function and trigger something (a deleter) when the object is destroyed, thus tying the action to the scope of the =finaliser=.
...
class finaliser {
  public:
    finaliser(Callable deleter):
        deleter_(std::move(deleter))
    {}

    finaliser(State initial_state, Callable deleter):
        deleter_(std::move(deleter)),
...
#+end_src
Some notes on =finaliser=:
 - A protected state is baked in to allow some customisation of the deleter's behaviour by extending =finaliser=.
 ...
 - Every special member function under the sun is deleted because a given deleter should only be called once.
...
Note that should the return value be ignored, the side effect is enacted immediately, resulting in the following output:
#+begin_src text
Live together.
...
This can be problematic but I have no solution, only a slight mitigation consisting in putting the =nodiscard= attribute to generate a compiler warning should a return value be ignored.
